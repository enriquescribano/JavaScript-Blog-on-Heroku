...
exports.new = function(req, res) {
    ...
    res.render('session/new', 
               { redir: req.query.redir || '/', visitantes: res.locals.visitas
               });
};
...
//Caducidad de sesión
//
//Destruye la session si ha pasado más de un minuto desde que fue invocada
//
exports.timer = function(req, res, next) {

    if(req.session.user != null){
        miFecha = new Date().getTime();
        var timer = (miFecha - req.session.user.time);
        if(timer > 60000) {
            delete req.session.user;
            req.flash('error', 'La sesión ha expirado, el usuario ha estado 1 minuto inactivo');
            res.redirect("/login");
        }
        else {
            req.session.user.time = miFecha;
        }
    }
    else {
